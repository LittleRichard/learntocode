...
VERTICAL_BOARD_STR = '|'
assert len(BLANK_STR) == len(PLAYER_1_STR) == len(PLAYER_2_STR) == len(PLAYER_3_STR)
...
def board_has_empty_spaces(board_state):

    for row in board_state:
        for col_val in row:
            if col_val == BLANK_STR:
                return True
...
def get_next_player(current_player):
    ...
    for idx in xrange(len(PLAYERS_IN_ORDER)):


        if PLAYERS_IN_ORDER[idx] == current_player:
            return PLAYERS_IN_ORDER[idx + 1]

        if idx == (len(PLAYERS_IN_ORDER) - 1):
            return PLAYERS_IN_ORDER[0]

    # if we get here... wtf?
    assert False, 'mystery player? current_player not in PLAYERS_IN_ORDER'

# YOUR PROGRAM ACTUALLY STARTS HERE

# create the board state that will track th
board_state = []
BOARD_HEIGHT = 4 # standard is 6
BOARD_WIDTH = 3 # standard is 7
...
while board_has_empty_spaces(board_state): 
...
print('Game over, no more open spaces')